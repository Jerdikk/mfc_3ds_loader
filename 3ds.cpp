...
bool CLoad3DS::Import3DS(t3DModel *pModel, const WCHAR *strFileName)
{
	...
	if(!m_FilePointer) 
	{
		wsprintf(strMessage, L"Unable to find the file: %s!", strFileName);
	...
	{
		wsprintf(strMessage, L"Unable to load PRIMARY chuck from file: %s!", strFileName);
		MessageBox(NULL, strMessage, L"Error", MB_OK);
		return false;
	}
